...
public class FirebaseUploader {
    ...
    public void uploadImage(Context context, Uri file) {
        compressAndUpload(context, "images", file);
    }
...
//                    }
    ...
    @SuppressLint("StaticFieldLeak")
    private void compressAndUpload(final Context context, final String child, final Uri file) {
        AsyncTask<Uri, Void, Uri> compressionTask = new AsyncTask<Uri, Void, Uri>() {
            @Override
            protected Uri doInBackground(Uri... files) {
                String filePathCompressed = null;
                Uri originalFileUri = files[0];
