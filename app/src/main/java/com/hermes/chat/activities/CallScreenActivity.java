...
public class CallScreenActivity extends BaseActivity {
    ...
    }/* extends BaseActivity implements SensorEventListener {
    ...
    private void saveLog() {
        if (!logSaved) {
            CallLogFireBaseModel callLogFireBaseModel = new CallLogFireBaseModel(user.getId(),
                    "", "", System.currentTimeMillis(), inOrOut,
                    isVideo, userMe.getId());
            String id = BaseApplication.getCallsRef().child(userMe.getId()).push().getKey();
            callLogFireBaseModel.setId(id);
            callLogFireBaseModel.setDuration(mCallDuration.getText().toString());
            ...
            rChatDb.copyToRealm(new LogCall(user, System.currentTimeMillis(), mCallDurationSecond,
                    isVideo, inOrOut, userMe.getId(), user.getId(), callLogFireBaseModel.getId(),
...
//                remoteVideo.setVisibility(View.INVISIBLE);
    ...
    private void updateUI() {
        ...
        if (call != null) {
            ...
            myTxtCalling.setText((isVideo ? " " + Helper.getCallsData(CallScreenActivity.this)
                    .getLblVideoCall() : " " + Helper.getCallsData(CallScreenActivity.this).getLblVoiceCall()));
