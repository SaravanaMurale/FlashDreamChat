...
import com.hermes.chat.fragments.UserMediaFragment;
    ...
    void groupUpdated(Group valueGroup) {
        if (group != null && group.getId().equals(valueGroup.getId())) {
            ...
            if (!group.getUserIds().contains(userMe.getId())) {
                userStatus.setText(getGroupData(ChatDetailActivity.this).getLblRemovedInfo());
...
//            group = intent.getParcelableExtra(EXTRA_DATA_GROUP);
        ...
        pickImage.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (group != null && group.getId().split(Helper.GROUP_PREFIX)[1].split("_")[1].equalsIgnoreCase(userMe.getId())) {
                    ...
                    alertDialog.setMessage(getProfileData(ChatDetailActivity.this).getLblGetImgFrom());
                    alertDialog.setPositiveButton(getProfileData(ChatDetailActivity.this).getLblCamera(), new DialogInterface.OnClickListener() {
                        @Override
                    ...
                    alertDialog.setNegativeButton(getProfileData(ChatDetailActivity.this).getLblGallery(), new DialogInterface.OnClickListener() {
                        @Override
                        public void onClick(DialogInterface dialogInterface, int i) {
                            dialogInterface.dismiss();
                            openImagePick();
                    ...
                    Toast.makeText(ChatDetailActivity.this, getProfileData(
                            ChatDetailActivity.this).getLblGroupAdminCanChange(), Toast.LENGTH_SHORT).show();
                }
                ...
                if (task.isSuccessful()) {
                    ...
                    groupRef.child(group.getId()).setValue(group).addOnSuccessListener(new OnSuccessListener<Void>() {
                        ...
                        public void onSuccess(Void aVoid) {
                            Toast.makeText(ChatDetailActivity.this, getProfileData(
                                    ChatDetailActivity.this).getLblUpdated(), Toast.LENGTH_SHORT).show();
                        }
                    });
                } else {
                    Toast.makeText(ChatDetailActivity.this, getProfileData(
                            ChatDetailActivity.this).getErrUnableToUpload(), Toast.LENGTH_SHORT).show();
                }
            }
    ...
    private void updateGroupNameAndStatus(String updatedName, String updatedStatus) {
        if (TextUtils.isEmpty(updatedName)) {
            Toast.makeText(this, getProfileData(ChatDetailActivity.this).
                    getErrEmptyName(), Toast.LENGTH_SHORT).show();
        } else if (TextUtils.isEmpty(updatedStatus)) {
            Toast.makeText(this, getProfileData(ChatDetailActivity.this).
                    getErrEmptyStatus(), Toast.LENGTH_SHORT).show();
